...
//
double * transfer(double **TwoD_array, int size);
...
int main(void) {
  ...
  if (webfile) {
	...
	// trasfer to a single one
	singleD_array = transfer(connectMatrix,websize);
...
double * transfer(double **TwoD_array, int size)
	...
	{  double * single = NULL;
		...
		int k = 0;
			...
			single = (double * )malloc(size*size * sizeof(double));
